...
class CuniAppSeeder extends Seeder
{
    ...
    private function seedUserData(): void
    {
         ...
         *  lapereaux_per    │    4  │    4  │    4  │    4  │    3  │    0
         *  → ~lapereaux     │  800  │  800  │  800  │  800  │   45  │    0
        ...
        $userConfigs = [
            // user_id => config
            1 => ['males' => 60, 'females' => 200, 'saillies' => 300, 'mises_bas' => 250, 'naissances' => 200, 'lapereaux_per' => 4, 'sales' => 80, 'notifs' => 20],  // Admin
            2 => ['males' => 60, 'females' => 200, 'saillies' => 300, 'mises_bas' => 250, 'naissances' => 200, 'lapereaux_per' => 4, 'sales' => 80, 'notifs' => 20],  // User1 – 1-month sub
            3 => ['males' => 60, 'females' => 200, 'saillies' => 300, 'mises_bas' => 250, 'naissances' => 200, 'lapereaux_per' => 4, 'sales' => 80, 'notifs' => 20],  // User2 – 3-month sub
            4 => ['males' => 60, 'females' => 200, 'saillies' => 300, 'mises_bas' => 250, 'naissances' => 200, 'lapereaux_per' => 4, 'sales' => 80, 'notifs' => 20],  // User3 – 6-month sub
        ...
        foreach ($userConfigs as $userId => $cfg) {
            ...
            // ── 4. MISES BAS ─────────────────────────────────────────────────
            // Use saillies with a positive palpation result first, then fill up
            ...
            // If not enough positives, take any saillies
    ...
    private function printLoginCredentials(): void
    {
        ...
        $this->command->line('║  Sub      : Active (1 year)                                      ║');
        $this->command->line('║  Dataset  : 🐇 HUGE  – 60 mâles │ 200 femelles │ ~800 lapereaux  ║');
        $this->command->line('║                        300 saillies │ 80 ventes │ 20 notifs       ║');

        $this->command->line('╠══════════════════════════════════════════════════════════════════╣');
        $this->command->line('║                                                                  ║');
        $this->command->line('║  👤  USER 1   –  user1@cuniapp.bj   /  User123!                 ║');
        $this->command->line('║  Subscription : 1 mois (active, ~25 jours restants)              ║');
        $this->command->line('║  Dataset  : 🐇 HUGE  – 60 mâles │ 200 femelles │ ~800 lapereaux  ║');
        $this->command->line('║                        300 saillies │ 80 ventes │ 20 notifs       ║');

        $this->command->line('║                                                                  ║');
        $this->command->line('║  👤  USER 2   –  user2@cuniapp.bj   /  User123!                 ║');
        $this->command->line('║  Subscription : 3 mois (active)                                  ║');
        $this->command->line('║  Dataset  : 🐇 HUGE  – 60 mâles │ 200 femelles │ ~800 lapereaux  ║');
        $this->command->line('║                        300 saillies │ 80 ventes │ 20 notifs       ║');

        $this->command->line('║                                                                  ║');
        $this->command->line('║  👤  USER 3   –  user3@cuniapp.bj   /  User123!                 ║');
        $this->command->line('║  Subscription : 6 mois (active)                                  ║');
        $this->command->line('║  Dataset  : 🐇 HUGE  – 60 mâles │ 200 femelles │ ~800 lapereaux  ║');
        $this->command->line('║                        300 saillies │ 80 ventes │ 20 notifs       ║');

        $this->command->line('║                                                                  ║');
        $this->command->line('║  👤  USER 4   –  user4@cuniapp.bj   /  User123!                 ║');
        $this->command->line('║  Subscription : 1 an (active)                                    ║');
        $this->command->line('║  Dataset  : 🐣 SMALL – 10 mâles │ 20 femelles │ ~45 lapereaux    ║');
        $this->command->line('║                        25 saillies │  5 ventes  │  5 notifs       ║');
