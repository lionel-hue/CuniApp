            ...
            <div class="card-body">
                <div class="settings-grid">
                    <div class="form-group">
                        ...
                        <p class="fw-semibold" style="font-family: 'JetBrains Mono', monospace; font-size: 15px;">{{ $femelle->code }}</p>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <p style="color: var(--text-secondary); font-size: 13px;">{{ $femelle->created_at->format('d/m/Y à H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="cuni-card" style="margin-top: 24px;">
            ...
            <div class="card-body">
                @php
                    ...
                    $totalLapereaux  = $femelle->naissances()->sum('nb_vivant') ?? 0;
                @endphp
                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 20px;">
                    ...
                    <div style="text-align: center; padding: 20px 16px; background: var(--surface-alt); border-radius: var(--radius-lg); border: 1px solid var(--surface-border);">
                        <div style="font-size: 28px; font-weight: 700; color: var(--accent-green);">{{ $naissancesCount }}</div>
                        <div style="font-size: 12px; color: var(--text-tertiary); margin-top: 4px;">Portées</div>
                    </div>
                    <div style="text-align: center; padding: 20px 16px; background: var(--surface-alt); border-radius: var(--radius-lg); border: 1px solid var(--surface-border);">
                        <div style="font-size: 28px; font-weight: 700; color: var(--accent-orange);">{{ $totalLapereaux }}</div>
                ...
                {{-- Recent saillies --}}
                @php
                    $recentSaillies = $femelle->saillies()
                        ->with('male')
                    ...
                    <div style="display: flex; flex-direction: column; gap: 8px;">
                        ...
                        <a href="{{ route('saillies.show', $saillie->id) }}" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; background: var(--surface-alt); border-radius: var(--radius); border: 1px solid var(--surface-border); text-decoration: none; color: inherit; transition: all 0.2s;" onmouseover="this.style.borderColor='var(--primary)'" onmouseout="this.style.borderColor='var(--surface-border)'">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                ...
                                <span style="font-size: 13px; font-weight: 500;">avec {{ $saillie->male->nom ?? '-' }}</span>
                            </div>
    ...
    <div class="detail-sidebar">
        ...
        <div class="cuni-card" style="margin-bottom: 20px;">
            ...
            <div class="card-body" style="text-align: center; padding: 28px 24px;">
                ...
                <span class="badge" style="background: {{ $etatColors[$femelle->etat] ?? $etatColors['Active'] }}; font-size: 14px; padding: 6px 18px;">
                    {{ $femelle->etat }}
                </span>
                @if($femelle->date_naissance)
                    <p style="font-size: 12px; color: var(--text-tertiary); margin-top: 12px;">
                        <i class="bi bi-calendar3"></i>
                        Née le {{ $femelle->date_naissance->format('d/m/Y') }}
                    </p>
                @endif
            </div>
        </div>
        ...
        <div class="cuni-card" style="margin-bottom: 20px;">
            ...
            <div class="card-body" style="display: flex; flex-direction: column; gap: 10px;">
                ...
                <a href="{{ route('mises-bas.index') }}?femelle_id={{ $femelle->id }}" class="btn-cuni secondary" style="width: 100%;">
